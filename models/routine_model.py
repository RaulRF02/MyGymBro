...
class Routine(db.Model):
    ...
    description = db.Column(db.Text, nullable=True)  # Brief explanation of the routine's purpose and benefits
    ...
    created_by = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Trainer or admin who created it
    assigned_to = db.Column(db.Integer, db.ForeignKey('users.id'),
                            nullable=True)  # User assigned to this routine (for custom routines)
    ...
    # Optional notes
    notes = db.Column(db.Text, nullable=True)  # Space for trainers or users to add comments
